...
import { connectUSBDevice, getCameraAccess, startCountdown, takePicture, downloadImage, saveImageToIndexedDB, selectUSBDevice } from '../controllers/Controller.js';
    ...
    const initializeCamera = async () => {
      try {
        if (Peripherie.hasExternCamera && !device) {
          console.log("Versuche, externe Kamera zu verbinden...");
          const selectedCamera = await selectUSBDevice(setDevice);
          if (selectedCamera) {
            await getCameraAccess(selectedCamera, videoRef, setVideoStreamActive);
          } else {
            console.log("Keine externe Kamera ausgewählt. Verwende Standardkamera...");
            await getCameraAccess(null, videoRef, setVideoStreamActive);
          }
        } else {
          console.log("Externe Kamera nicht verfügbar. Verwende Standardkamera...");
          await getCameraAccess(null, videoRef, setVideoStreamActive);
        }
      } catch (error) {
            ...
            {videoStreamActive && showButtons && (
              <>
                <div className="button-container">
                  ...
                  />
                  {/* 
                  <button className="usb-device-button" onClick={() => selectUSBDevice(setDevice)}>
                    USB-Gerät auswählen
                  </button>
                  */}
                  ...
                  {!photoTaken && showButtons && (
                    <div className="footer-text">Timer: {timerValue} Sekunden</div>
                  )}
                </div>
              </>
            )}
